...
1. Values for parent relation fields  (for example, an `organizationId` field on a `members`  model)
2. Default values for fields (if you didn't explicitly define what the value of a field should be anywhere)
3. Values given for fields using the `models` option for `SnapletClient`, example:
...
4. Values given for fields using the `models` option for a plan, example:
...
5. Fields specified earlier in the same object for a particular model, example:
...
We've removed `modelSeed` parameter that we pass to generate functions in favor of the new `data` mentioned above. 
