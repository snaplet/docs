...
> Learn how to add a `seed.mts` to your project, using our [quick start](https://docs.snaplet.dev/getting-started/quick-start/seed) guide.
...
You can also use the restore command to restore data into your database. Just replace the `seed` command with `restore`:

```bash >_&nbsp;terminal
prisma migrate dev && snaplet restore --no-reset --no-schema
```
