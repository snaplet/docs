...
<Callout type="info">
  A user account is _not required_ - you only need a user account if you want to
...
#### Follow Nullable Relations (followNullableRelations: boolean | { [tableName | '$default']: boolean | { [relationName | '$default']: boolean } }) (default=true)
...
#### Max cycles loop (maxCyclesLoop: number | { [tableName | '$default']: number | { [relationName | '$default']: number } }) (default=1)
...
#### Max Children Per Node (maxChildrenPerNode: number | | { [tableName | '$default']: number | { [relationName | '$default']: number } }) (default=unlimited)
...
<Callout type="info">
  Note that the `precent` / `rowLimit` specified in the subset config may not be
  exact. The actual row count of the data is affected by the relationships
  between the tables. As such, a 5% subset specified against a specific table
  may ultimately include more than 5% of the actual database.
</Callout>
