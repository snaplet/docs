...
```ts
const run = () =>
  snaplet.$transaction(async (snaplet) => {
...
And similarly, fields specified for a model when instantiating `SeedClient` (`name` in the example below) will be available in `data` for
...
await snaplet.users([{
  // `name` was given when instantiating `SeedClient`, so it'll be available here
  email: ({ data }) => `${data.name}@example.org`.toLowerCase()
}])
```
...
3. Then values given for fields using the `models` option for `SeedClient` (as shown below) will be generated next:
