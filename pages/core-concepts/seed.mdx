...
This store is very useful when you want to [connect](/core-concepts/seed#connecting-data) your models, reusing existing data instead of generating new one.


### Static or dynamic data, we got you covered

If you want to use static data, you can directly pass an array to the models methods.
...
When `true`, the plan will automatically connect models relationships to fulfill to the client's global store if possible. The corresponding model will be picked randomly (but deterministically, we're using `copycat.oneOf` method under the hood).
