...
If you didn't create a snapshot with Snaplet Cloud, you'll need to do so, or create a snapshot manually. More advanced users can also capture a snapshot from inside the CLI. Go to our [capture](/core-concepts/capture) page for more info on how to do this.
...
It's possible to omit any of the steps above during snapshot restoration by using an opt-out flag. For instance, when restoring to a [Supabase database](/recipes/supabase), it's necessary to skip resetting the database itself.
