...
Instead, we're introducing a new package called `@snaplet/client`. We extracted the data client into its own package so you can use it everywhere in your codebase!
Seed scripts or tests, data are now one import away! Let's see how to migrate to it.

First, install the new package:

```bash >_&nbsp;terminal
npm install -D @snaplet/client
```
...
To know more about the data client, check out the [documentation](/getting-started/quick-start/generate).
