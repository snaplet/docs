...
Previously, you would instantiated a class like this:
...
Also, you noticed that we changed our naming convention, from `snaplet` to `seed`. We think it's more easy to understand.
...
```ts
await seed.posts(x => x(3, ({ index, seed }) => ({
  title: `Post #${index}`,
  content: `This post's seed is ${seed}`
})));
...
await seed.posts([
  ...
  },
  {
    ...
    author: ({ seed }) => {
      const createdAt = new Date(copycat.dateString(seed))
