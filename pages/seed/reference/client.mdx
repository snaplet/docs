...
All the **Example type** are based on the `users` model.
...
- **Example type:** `UserPlanInputs`

Provides a description of the data to seed.

The `UserPlanInputs` type is a union of two types:
- `UserInputs`: an object that describes the data to seed.
- `(ctx: PlanContext) => UserInputs`: a callback receiving the plan's context and which returns an object that describes the data to seed.
...
type UserPlanInputs = UserInputs | ((ctx: PlanContext) => UserInputs);
```
...
```ts
await seed.users([
  (ctx) => ({ username: `Snappy ${ctx.index}` }),
]);
```

The `UserInputs` type is a record of the model's fields.
...
type UserInputs = {
  ...
  email?: string | ((ctx: FieldContext) => string | Promise<string>);
  username?: string | ((ctx: FieldContext) => string | Promise<string>);
  // relationships (children fields)
  posts?: Array<PostPlanInputs> | xCallback<PostPlanInputs>;
...
await seed.users([
  {
    username: "Snappy",
    email: (ctx) => `${ctx.data.username!}@snaplet.dev`,
  },
]);
```
...
await seed.users([
  {
    username: async (ctx) => {
      return await fetchUsername(ctx.seed);
    },
  },
]);
```

Children field as a static array:
