...
However, if the tables are from the same schema (e.g. `public.user` and `public.users`), we do not have a way to resolve the conflict, and need you to resolve the conflict by specifying `overrides` for this in `seed.config.ts`. For example:
...
#### Fix support for reading `fingerprint` from `seed.config.ts`
We previously exposed types for providing `fingerprint` in `seed.config.ts`, but did not actually read this from the config in `@snaplet/seed`.
...
`snaplet.$resetDatabase()` **now respects** `select` **in** `seed.config.ts`

If you chose to exclude some schemas or tables in `seed.config.ts` with `select`, `snaplet.$resetDatabase()` will now exclude them, and truncate all the remaining tables of the remaining schemas in your database:
