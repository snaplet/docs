...
#### Bug Fixes
* Fix a bug causing the autocomplete for tables in the `select` field of the `seed.config.ts` to not show up.
* Fix a bug causing models defined values to be ovewritten on some filed for columns with default value in database.
* Fix a bug causing the generated client assets to be lost after a multiples `npm install`

#### Features
* Add an `--access-token` option to the login command to allow logging in with an already existing access token.
* Add a `--config` option to `sync` command to allow specifying where the `seed.config.ts` is located in monorepo setup.
</Step>
