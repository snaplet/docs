...
The adapter is now persisted to `.snaplet/config.json`.
  ...
  ```
- **Deeper Prisma integration**: When using Prisma adapter, we are now parsing your `prisma.schema` and automatically reflect your model and field names in the generated Seed Client.

  To have this change applied, regenerate your Seed Client using `npx @snaplet/seed sync`.
