...
You already followed the [installation guide](/seed/getting-started/installation) and have `@snaplet/seed` available locally.
...
In order to do that, we change `dryRun` to `false`. Now, instead of logging the SQL statements to the
...
Learn more about how `@snaplet/seed` works and define more complex plans in our [seed reference guide](/seed/core-concepts)
