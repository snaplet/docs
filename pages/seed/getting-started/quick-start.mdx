...
In order to do that, change `dryRun` to `false`. Now, instead of logging the SQL statements to the
console, the data will be inserted into your database:
...
```
# * fetches the new structure from your database
# * uses it to regenerate @snaplet/seed assets
npx @snaplet/seed dev
```

## Next steps

Learn more about how `@snaplet/seed` works and define more complex plans in our [seed reference
guide](/seed/core-concepts)
