...
Once you save the edited file, the `init` command will continue and create a `seed.mts` example file in your project folder.
...
You can hook into [Supabase seeding workflow](https://supabase.com/docs/guides/cli/local-development#add-sample-data) using the `dryRun` option for client creation in the `seed.mts` file:
...
For a full example of a project using `@snaplet/seed` with Supabase, check out the [example project](https://github.com/snaplet/examples/tree/main/seed/supabase-twitter-clone)