...
This guide will take you through the process of using `@snaplet/client` to create realistic, production-like data.
...
It's worth noting what we're not explicitly defining - that each comment needs to have a separate, unique author, for instance. We're assuming that `@snaplet/client` will fill in the blanks based on your schema and generate realistic data as needed.

### Describing your requirements

In order to have Snaplet generate this data for your blog, we need to describe these requirements to `@snaplet/client`.

<CH.Scrollycoding className="generate-tutorial-configuration-file">

First, we need to create a new seed script file that imports `@snaplet/client`.
...
- If you chose to have Snaplet generate an example for you during `snaplet setup`, you should have `seed.mts` file in your project that looks something like the code below.
- If you chose to not have Snaplet generate an example, create a `seed.mts` file in your project (the directory where you ran `snaplet setup`) that looks like this:
...
These functions return what we call a _plan_. When describing a plan, you always start from a "root" model - a model where we would start describing how to generate seed data, along with data for its related models (more on this soon).
...
As mentioned earlier, these 3 users were automatically generated to satisfy the requirement of 3 comments. You didn't need to define them - `@snaplet/client` did it for you, because it knows your schema relationships and can fulfill them automatically.
...
Learn more about how `@snaplet/client` works and define more complex plans in our [generate reference guide](/core-concepts/generate)
