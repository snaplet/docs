import { Tabs, Tab } from "nextra/components";

# Installation

## For generating seed data

First, you'll need [Node.js](https://nodejs.org/en/) and [postgres](https://www.postgresql.org/) installed on your system.

Once you have these, installation should be as simple as running this following in your command line:

```bash >_&nbsp;terminal
# Set up Snaplet with npx
npx snaplet setup
```

Snaplet will ask you a few questions set up snaplet for your project. Along the way, it try to detect the package manager used for your project and use this
to download the snaplet npm packages needed for your project.

## For snapshots
Snaplet can either be self-hosted or accessed as a web app via our cloud service, Snaplet cloud. The choice is yours!

We recommend beginning with [Snaplet cloud](https://snaplet.dev). Sign up for free and start utilizing your data in mere minutes!

For those looking to delve deeper or self-host Snaplet, try our user-friendly `snaplet setup` CLI wizard!

<Tabs items={["npx", "curl"]}>
  <Tab>
  ```bash >_&nbsp;terminal
  # Set up Snaplet with npx
  npx snaplet setup
  ```
  </Tab>
  <Tab>
  ```bash >_&nbsp;terminal
  # Set up Snaplet as a static binary
  curl -sL https://app.snaplet.dev/get-cli/ | bash
  snaplet setup
  ```
  </Tab>
</Tabs>


When you run `snaplet setup`, you will be asked a few questions to complete setup, and generate a [`snaplet.config.ts`](/reference/configuration) configuration file for your project.

## Manual setup

### Installing dependencies

Snaplet requires to following to be present in your project:
* a `package.json`: to determine where to install snaplet packages to
* a package manager lockfile to be present for your project - to detect which package manager you are using

If you do not have a `package.json`, you can add one to your project with:

```bash >_&nbsp;terminal
npm init -y
```

We look for a package manager lockfile for npm, pnpm, yarn, or bun. If you do not have a lockfile for your project, or you are using a different package manager, you can still install snaplet npm packages manually:

<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab>
  ```bash >_&nbsp;terminal
  npm install --save-dev snaplet @snaplet/seed
  ```
  </Tab>
  <Tab>
  pnpm --save-dev snaplet @snaplet/seed
  ```
  </Tab>
  <Tab>
  ```bash >_&nbsp;terminal
  yarn add --dev snaplet @snaplet/seed
  ```
  </Tab>
    <Tab>
  ```bash >_&nbsp;terminal
  bun add --dev snaplet @snaplet/seed
  ```
  </Tab>
</Tabs>
