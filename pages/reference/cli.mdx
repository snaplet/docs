...
The `snaplet generate` command is used to regenerate the assets needed by [`@snaplet/seed`](/core-concepts/seed).

You would typically run this whenever you have changed your database structure, so that the assets used by `@snaplet/seed` correspond to your new database structure.
