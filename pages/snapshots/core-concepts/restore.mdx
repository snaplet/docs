...
If you didn't create a snapshot with Snaplet Cloud, you'll need to do so, or create a snapshot manually. More advanced users can also capture a snapshot from inside the CLI. Go to our [capture](/snapshots/core-concepts/capture) page for more info on how to do this.
...
What's next? If you haven't already, connect Snaplet to your own database. You can [self-host](/snapshots/guides/self-hosting) Snaplet in your own trusted infrastructure as well.
...
It's possible to omit any of the steps above during snapshot restoration by using an opt-out flag. For instance, when restoring to a [Supabase database](/snapshots/recipes/supabase), it's necessary to skip resetting the database itself.
