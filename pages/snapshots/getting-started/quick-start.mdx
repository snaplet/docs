...
*If you're uncomfortable connecting to your actual production database, use your staging or QA database, or the Snaplet demo database. Supabase users should select the Supabase option and follow the [Supabase tutorial](/snapshots/recipes/supabase) for assistance.*
...
Once done, click '**Test my connection**' to verify that Snaplet is able to connect to your database, and proceed. If you see a warning about read-only access to your database, make sure you're using [read-only role](/snapshots/guides/postgresql/#create-a-read-only-role) credentials for Snaplet.
...
For more control over how your snapshot captures data from your source database, including selecting and excluding specific tables, check out the [capture guide](/snapshots/core-concepts/capture#select-data)

To learn how to modify your data transformations, including how to use [`@snaplet/copycat`](https://github.com/snaplet/copycat) to seed realistic, deterministic values, check out the [data transformation guide](/snapshots/core-concepts/capture#select-data)

For more options and granular control over snapshot restoration, check out the Snaplet [restore guide](/snapshots/core-concepts/restore#more-granular-control-over-restorations).